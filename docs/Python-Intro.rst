...
**Note**: For NumPy arrays, convert categorical features to non-negative ``int`` values before
constructing the ``Dataset``. For dataframe inputs with a categorical dtype, integer codes are
extracted automatically; with the default
