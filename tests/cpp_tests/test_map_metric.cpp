...
TEST_F(MapMetricTest, CalMapAtK) {
  ...
  LightGBM::MapMetric::CalMapAtK(
    {2, 5}, 2,
    new LightGBM::label_t[]{0.4f, 0.6f, 0.8f, 0.2f},
    new double[]{1., 3., 2., 4.},
    4, &out
  );
  EXPECT_NEAR(
  ...
  LightGBM::MapMetric::CalMapAtK(
    {2, 5}, 2,
    new LightGBM::label_t[]{0.1f, 0.9f, 0.8f},
    new double[]{6., 1., 2.},
    3, &out
  );
  EXPECT_NEAR(
...
TEST_F(MapMetricTest, Eval) {
  ...
  metadata.SetQuery(
    new LightGBM::data_size_t[]{4, 3, 2},
    3
  );
  metadata.SetLabel(
    new LightGBM::label_t[] {
  ...
  std::vector<double> out = metric.Eval(
    new double[]{
      1., 3., 2., 4., // query 0
      6., 1., 2., // query 1
      3., 4. // query 2
